...
from .config import ModelConfig, TASK_NAMES, OLIGOMER_MAX_REPEATS
...
def load_checkpoint(
    ...
    return model_config, scaler, model, ckpt
...
def predict_pair(
    ...
    """
    model_config, scaler, model, _ = load_checkpoint(checkpoint_path)
    featurizer = create_featurizer()

    cyc1, mol1 = cyclize_polymer_with_cp_marking(
        smiles_1, depth=model_config.depth, max_repeats=max_repeats)
    cyc2, mol2 = cyclize_polymer_with_cp_marking(
        smiles_2, depth=model_config.depth, max_repeats=max_repeats)
...
def predict_batch(
    ...
    if valid.empty:
        raise ValueError(
            "No valid polymer pairs after cyclization; "
            "check Polymer_1/Polymer_2 SMILES (each must contain exactly two '*')")
